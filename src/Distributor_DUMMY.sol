...
contract Distributor_DUMMY is Ownable {
    ...
    modifier isOpen(bool earlyMint) {
        if (earlyMint) {
            require(block.timestamp > earlyMintStart, "early mint not open");
        } else {
            require(block.timestamp > publicMintStart || publicSaleOpen, "public mint not open");
        }
        _;
    }
    ...
    function setMintTimes(uint256 newEarlyMint, uint256 newPublicMint) public onlyOwner() {
        earlyMintStart = newEarlyMint;
        publicMintStart = newPublicMint;
    }
